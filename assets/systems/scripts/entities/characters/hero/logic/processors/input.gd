...
func set_ledges(processor: Node, surface: Node) -> void:
	processor.body_entered.connect(surface.append)
	processor.body_exited.connect(surface.remove)
...
func set_directing(hero: CharacterBody2D) -> void:
	...
	var ledges: Node = processors.input.platforming.jump.ledges

	set_floors(hero.logic.detectors.platforming.floors, surface)
	set_ledges(hero.logic.detectors.platforming.platforms.ledges, surface)
