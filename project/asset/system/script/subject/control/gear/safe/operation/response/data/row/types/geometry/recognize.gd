...
func regex_search(expression: RegEx, object: Dictionary, type: String, converter: Callable) -> void:
	var result: RegExMatch = expression.search(object.data.get_string_from_ascii())
	if result:
		object.row.append(converter.call(result))
		# object.row.append(vector(result, 1, 2)) ??
	else: _end(object.responses, type, "invalid_value")

func _geometry(object: Dictionary, type: String, converter: Callable) -> void:
	var expression = RegEx.new()
	if expression.compile(regex[type]) == OK:
		regex_search(expression, object, type, converter)
...
func path(object: Dictionary) -> void: object.row.append(object, PackedVector2Array())
...
func circle(object: Dictionary) -> void: return line(object, "CIRCLE")

func _todo(type: String) -> void: print("TODO '%s' type implementation" % type)
func tsvector(_object: Dictionary) -> void: _todo("TSVECTOR")
func tsquery(_object: Dictionary) -> void: _todo("TSQUERY")
