...
func _number16(responses: BackendResponses, seek: int, start: int) -> int:
	return responses.reverse(seek, start).get_u16()

func _number32(responses: BackendResponses, seek: int, start: int) -> int:
...
func row(object: Dictionary) -> void:
	var responses: BackendResponses = object.responses
	query_result.fields_number = get_fields_number(responses)
	for _index in query_result.fields_number:
		...
		var fields: Dictionary = { # Get the ... if field is specific table column: otherwise 0
			"table_object_id": _number32(responses, 0, 5), # ... table object ID ...
			"column_index": _number16(responses, 4, 2), # ... column attribute number ...
			"type_object_id": _number32(responses, 6, 4), # field's data type object ID
			"data_type_size": _number16(responses, 10, 2), # data type size
			"type_modifier": _number32(responses, 12, 4), # type modifier. The meaning is type-specific.
			"format_code": _number16(responses, 16, 2) # field used format code. 0 (text, default) / 1 (binary).
...
func parameter(object: Dictionary) -> void: 
	var responses: BackendResponses = object.responses
	var types: Array = []
