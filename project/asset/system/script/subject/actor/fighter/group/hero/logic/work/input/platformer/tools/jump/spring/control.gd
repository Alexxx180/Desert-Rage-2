...
func jump(jumped: bool) -> void:
	...
	layers.context(!jumped).collide_main().collide(Lay.BORDERS)
...
func landing_crash() -> void:
	if not slide.above(ground.world_y): # hero.position.y = ground #print("GROUND: ", ground, " - Y: ", mode.hero.position.y, " - H: ", delta * height) # landing.emit()
		land()

func landing_manual() -> void:
	if slide.above(ground.world_y) and not platform.is_colliding() and Input.is_action_just_pressed("run"): # landing.emit()
		land() # height > 0
