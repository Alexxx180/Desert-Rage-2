...
func green(segment: float) -> Color:
	return Color.from_rgba8(NO, SET, int(SET * decide("BLUE", "GREEN", segment)))

func yellow(segment: float) -> Color:
	var color: float =  1.0 - decide("GREEN", "YELLOW", segment)
	return Color.from_rgba8(int(color * SET), SET, NO)

func red(segment: float) -> Color:
	return Color.from_rgba8(SET, int(SET * decide("YELLOW", "RED", segment)), NO)

func diffuse() -> Color:
	return Color.from_rgba8(NO, NO, NO, NO)

func get_color(segment: float) -> Color:
	if segment <= colors.RED: return Color.from_rgba8(SET, NO, NO)
	if segment >= colors.BLUE: return Color.from_rgba8(NO, SET, SET)
