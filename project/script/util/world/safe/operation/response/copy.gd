...
func _buffer(seek: int, start: int, end: int) -> StreamPeerBuffer:
	return backend.responses.reverse(seek, start, end)

func format_column_codes() -> void: # Must be 0 "text" / 1 "binary"
	var columns_to_copy: int = _buffer(1, 7, 10).get_16()
	for number in columns_to_copy:
		var seek: int = 2 * number + 3
		var code: int = _buffer(seek, 10, 13).get_16()
...
func _get_overall_copy_format_code() -> int: # Separation: 0. text as "rows [\r]\n, columns \t, etc"; 1. binary as "DataRow".
	return _buffer(0, 5, 7).get_8() # See COPY for more information.

func response(type: String) -> void: # Followed by copy data.
	backend.responses.buffer.renew()
	_get_overall_copy_format_code()
	format_column_codes()
	backend.connection.note.warn("no_support", " Copy" + type + "Response")

func get_stream_data_part() -> PackedByteArray:
	return backend.responses.fragments.word(5)
